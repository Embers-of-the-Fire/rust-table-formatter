...
use table_formatter::table::CellPadding;
...
fn test_table_default_builder_raw() {
    let table_header: Vec<TableCell> = vec![
        TableCell::new(Cell::TextCell("Cell Row".into())).with_width(20).with_padding(CellPadding::new(2, 0)),
    ...
    let table_cells: Vec<Vec<TableCell>> = (0..=3_u8)
        ...
        .collect_vec();
    let table = Table::from_data(table_header, table_cells).with_border(Border::TOP.mixin(&Border::BOTTOM));
